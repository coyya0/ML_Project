...
int main(int argc, char const *argv[])
{
    ...
    if(m == n && n == k)
    {
        // TODO 13
        printf("transpose\n");
        gpu_matrix_transpose<<<dimGrid, dimBlock>>>(d_b, t_b,n,n);
        gpu_square_matrix_mult<<<dimGrid, dimBlock>>>(d_a,t_b,d_c,n); // transpose 후 곱셈연산 = 나눗셈
    }
    else
    {
        // TODO 14
    ...
    cudaMemcpy(h_b,t_b, sizeof(int)*m*k, cudaMemcpyDeviceToHost); // div연산
